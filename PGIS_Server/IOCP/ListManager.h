...
template <class T>
void CListManager<T>::TraversalItem(const DWORD dwInstance,const DWORD dwClientContext,PFN_ON_TRAVERSAL pfnTraversal)
{
	WaitToRead(INFINITE);
	listIterator listIter = begin();
	while (listIter != end())
	{
		pfnTraversal(dwInstance,dwClientContext,DWORD(&(*listIter)));
		++listIter;
	}
	DoneReading();
}
