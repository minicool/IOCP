...
//////////////////////////////////////////////////////////////////////
#include "StdAfx.h"
#include "TcpMainServer.h"
#include "MemoryPool.h"

extern CWriteLogThread gWriteLogApp;

extern ::CCriticalSection g_csRecvServerList;
extern CList<RECV_SERVERDATA, RECV_SERVERDATA&> g_RecvServerList;
...
void CTcpMainServer::TcpDisConnect()
{
	...
	case SOCKET_SERVER_GPSCHECK_TYPEID:
		strTemp.Format("%s [SYS] 主控与巡防管理服务器的TCP连接断开\n",
	...
	case SOCKET_SERVER_LOCATION_TYPEID:
		strTemp.Format("%s [SYS] 主控与地理位置服务器的TCP连接断开\n",
			CTime::GetCurrentTime().Format("%H:%M:%S"));
...
{
	if(AppID != SOCKET_SERVER_GPSCHECK_TYPEID && AppID != SOCKET_SERVER_LOCATION_TYPEID)
	{
		return FALSE;
	}
	...
	if(m_bStartOk)
	{
		if(AppID == SOCKET_SERVER_GPSCHECK_TYPEID)
		{
			strTemp.Format("%s [SYS] 巡防管理服务已连接\n",
				CTime::GetCurrentTime().Format("%H:%M:%S"));
		}
		else
		{
			strTemp.Format("%s [SYS] 地理位置服务已连接\n",
				CTime::GetCurrentTime().Format("%H:%M:%S"));
		}
		gWriteLogApp.WriteLog(FOR_ALARM,strTemp);
		return TRUE;
    ...
    if( nRes != TCPCLIENT_CONNECTSUCCESS )
    {
		...
		{
		case SOCKET_SERVER_GPSCHECK_TYPEID:
			strTemp.Format("%s [SYS] 连接巡防管理服务器失败res:%d,IP:%s,端口:%d\n",
				CTime::GetCurrentTime().Format("%H:%M:%S"),nRes,strIPAddress,nPort);
			break;
		case SOCKET_SERVER_LOCATION_TYPEID:
			strTemp.Format("%s [SYS] 连接地理信息服务器失败res:%d,IP:%s,端口:%d\n",
				CTime::GetCurrentTime().Format("%H:%M:%S"),nRes,strIPAddress,nPort);
			break;
		default:
			break;
	...
	{
	case SOCKET_SERVER_GPSCHECK_TYPEID:
		strTemp.Format("%s [SYS] 连接巡防管理服务器成功,IP:%s,端口:%d\n",
			CTime::GetCurrentTime().Format("%H:%M:%S"),strIPAddress,nPort);
		break;
	case SOCKET_SERVER_LOCATION_TYPEID:
		strTemp.Format("%s [SYS] 连接地理信息服务器成功,IP:%s,端口:%d\n",
			CTime::GetCurrentTime().Format("%H:%M:%S"),strIPAddress,nPort);
		break;
	default:
		break;
...
void CTcpMainServer::Stop()
{
	if(m_bStartOk)
	{
		...
		CString strTemp;
		if(m_nAppID == SOCKET_SERVER_GPSCHECK_TYPEID)
		{
			strTemp.Format("%s [SYS] 与巡防管理服务器TCP连接断开\n",
				CTime::GetCurrentTime().Format("%H:%M:%S"));
		}
		else
		{
			strTemp.Format("%s [SYS] 与地理位置服务器TCP连接断开\n",
				CTime::GetCurrentTime().Format("%H:%M:%S"));
		}
		gWriteLogApp.WriteLog(FOR_ALARM,strTemp);
	}
...
int CTcpMainServer::SendData(const char *sendBuf,int nSendLen)
{
	...
	if(m_bStartOk)
	{
		...
		if(nLen == TCPCLIENT_ERROR_SEND )
		{
			...
			if(m_nAppID == SOCKET_SERVER_GPSCHECK_TYPEID)
			{
				strTemp.Format("%s [SYS] 向巡防管理服务器发送数据失败,TCP连接断开\n",
					CTime::GetCurrentTime().Format("%H:%M:%S"));
			}
			else
			{
				strTemp.Format("%s [SYS] 向地理位置服务器发送数据失败,TCP连接断开\n",
					CTime::GetCurrentTime().Format("%H:%M:%S"));
			}
	...
	if(m_bStartOk)
	{
		if(m_nCheckTimeValue++ >= 10)
		{
			...
			if(m_nTickCount >= 3)
			{
				...
				if(m_nAppID == SOCKET_SERVER_GPSCHECK_TYPEID)
				{
					gWriteLogApp.WriteLog(FOR_ALARM,"%s <PATR>巡防管理服务器TCP连接心跳超时\n",
						CTime::GetCurrentTime().Format("%H:%M:%S"));
...
void CTcpMainServer::ReportWebServerUpDataLocation(int nQueryIndex,char *strDeviceCode ,char *strLocation)
{
