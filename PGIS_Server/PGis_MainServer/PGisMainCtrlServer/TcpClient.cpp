...
//////////////////////////////////////////////////////////////////////
//
BOOL CTcpClient::AnalyseRecvData(const char *recvData ,int nRecvLen,int nAppID)
{
...
//     return TRUE;
	...
	{
		int nHeadPoint = -1;
		char *tempHeadCurPoint = pCurPoint;
		...
		if (-1 == nHeadPoint)
		{
			gWriteLogApp.WriteLog(FOR_ALARM,"%s [CTcpClient::ProcessData]包头错 %s,没有找到\n",CTime::GetCurrentTime().Format("%H:%M:%S"),tempHeadCurPoint);
			nSpaceLen = 0;
			break;
		}
		
		//查找包尾
		ptempCurPoint = pCurPoint;
		int nPoint = 0;
		for(int i = 0 ; i < nSpaceLen; i++ )
		{
			if( ptempCurPoint[i] == PACKAGE_TAIL_DATA_ETX)
			{
				nPoint = i;
				break;
			}
		...
		if( nPoint > 0 )
		{
			...
			if( nPackageLen < MAX_DATA_PACKAGE_LEN )
			{
				pCurPoint[nPoint] = NULL;
				TcpClientDataProcess(pCurPoint+1,nPackageLen-2,nAppID);
			}
			else
			{
				gWriteLogApp.WriteLog( FOR_ALARM,_T("%s [CTcpClient::ProcessData]包长度超过允许的最大值,len:%d.\n"),
					CTime::GetCurrentTime().Format("%H:%M:%S"),
					nPackageLen);
			}
			//剩下的数据继续分包
		...
		else
		{
			if( nSpaceLen > MAX_DATA_PACKAGE_LEN )
			{
				gWriteLogApp.WriteLog( FOR_ALARM,_T("%s [CTcpClient::ProcessData]超长的数据包,len:%d.\n"),
					CTime::GetCurrentTime().Format("%H:%M:%S"),nSpaceLen );
...
//GIS客户端 数据长度验证 
BOOL CTcpClient::TcpClientDataProcess(const char *recvData, int nRecvLen,int nAppID)
{
...
//GIS客户端发送过来的数据  
void CTcpClient::DispatchGisClientRecvData(int CmdType,const char *RecvData, int nLen)
{
...
void CTcpClient::StartTcpTickTimer(int nConnState ,int nTimerVal)
{
...
void CTcpClient::SetSocketConn(int nSocketID,const char *strIp)
{
...
void CTcpClient::StopTcpTickTimer()
{
    m_bStartTimerFlag = FALSE;
}

int CTcpClient::CheckTimerOver()
{
