...
using namespace std;
 ...
 string CXMLParse::_FormatStringFromBin(char * pData,int nLen)
{
	CString strReturn;
...
void CXMLParse::_FormatBinFromString(const char * strSource,char** ppData,int* pLen)
{
	*ppData=NULL;
	*pLen = 0;
	...
	if(nLen<=0)
		return;
	char* pSrc = (char*)strSource;
	char* pTmp = pSrc;
	*pLen = nLen/2;
	if(nLen%2)*pLen ++;
	*ppData = new char[*pLen];
	int nIndex = 0;
	while(pTmp-pSrc<nLen)
	{
		char h = *pTmp;
		char l = *(pTmp+1);
		int nH = atoi(&h);
...
void CXMLParse::GetData(char** ppData,int* pLen)
{
	string str = GetData();
	_FormatBinFromString(str.c_str(),ppData,pLen);
}

void CXMLParse::GetDoc(char** ppBuf,int* pnLen)
{ 
	*pnLen = this->m_strDoc.GetLength();
	*ppBuf = NULL;
	if(*pnLen>0)
	{
		*ppBuf = new char[*pnLen];
		memcpy(*ppBuf,this->m_strDoc,*pnLen);
	}
}
void CXMLParse::operator=( const CXMLParse& markup )
{
...
bool CXMLParse::SetDoc(char* pBuf,int len)
{
	this->m_strDoc.Empty();
	memcpy(this->m_strDoc.GetBufferSetLength(len),pBuf,len);
	//_remote_header();
	return x_ParseDoc();
}

void CXMLParse::_remote_header()
{
	CString header = "<?xml version=\"1.0\" encoding=\"utf-8\" ?>";
	this->m_strDoc.Replace(header,"");
}

bool CXMLParse::SetDoc( const char * szDoc )
{
	// Set document text
	if ( szDoc )
		m_strDoc = szDoc;
...
bool CXMLParse::Load( const char * szFileName )
{
...
bool CXMLParse::LoadData( const char * lpData)
{
	m_strDoc = lpData;	
	return x_ParseDoc();
}

bool CXMLParse::ReadTextFile( const char * szFileName, CString& strDoc, CString* pstrError, int* pnFlags )
{
...
bool CXMLParse::Save( const char * szFileName )
{
	return WriteTextFile( szFileName, m_strDoc, &m_strError, &m_nFlags );
}

bool CXMLParse::WriteTextFile( const char * szFileName, CString& strDoc, CString* pstrError, int* pnFlags )
{
...
bool CXMLParse::FindElem( const char * szName )
{
...
bool CXMLParse::FindChildElem( const char * szName )
{
...
string CXMLParse::EscapeText( const char * szText, int nFlags )
{
...
string CXMLParse::UnescapeText( const char * szText, int nTextLength /*=-1*/ )
{
...
int CXMLParse::x_ParseElem( int iPosParent, TokenPos& token )
{
				...
				if ( m_strError.IsEmpty() )
				{
					if ( nTypeFound == 0 )
						m_strError.Format( _T("End tag '%s' at offset %d does not match start tag '%s' at offset %d"),
							x_GetToken(token).c_str(), token.nL-1, aNodes.Top().strMeta, pElem->nStart );
					else
...
bool CXMLParse::x_FindAny( const char * szDoc, int& nChar )
{
...
bool CXMLParse::x_FindName( CXMLParse::TokenPos& token )
{
...
string CXMLParse::x_GetToken( const CXMLParse::TokenPos& token )
{
...
int CXMLParse::x_FindElem( int iPosParent, int iPos, const char * szPath ) const
{
...
int CXMLParse::x_ParseNode( CXMLParse::TokenPos& token, CXMLParse::NodePos& node )
{
...
int CXMLParse::x_RemoveElem( int iPos )
{
...
void CXMLParse::x_LinkElem( int iPosParent, int iPosBefore, int iPos )
{
...
int CXMLParse::x_UnlinkElem( int iPos )
{
...
int CXMLParse::x_ReleasePos( int iPos )
{
...
int CXMLParse::x_ReleaseSubDoc( int iPos )
{
...
void CXMLParse::x_CheckSavedPos()
{
...
void CXMLParse::x_AdjustForNode( int iPosParent, int iPos, int nShift )
{
...
bool CXMLParse::x_AddNode( int nNodeType, const char * szText, int nFlags )
{
...
void CXMLParse::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
