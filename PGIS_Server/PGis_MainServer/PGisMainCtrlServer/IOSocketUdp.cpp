...
BOOL CIOSocketUdp::Start(char* szIPAddr, unsigned short usPort, LPFUDPDATACALLBACK lpfUdpCallBack,DWORD dwInstance)
{
	_ASSERT(m_hSocket == INVALID_SOCKET);
	...
	{
		gWriteLogApp.WriteLog(FOR_DEBUG, TEXT("CIOSocketUdp::Start bind ip:%s,Port:%d failed\n"),szIPAddr, usPort);
		return FALSE;
	}
